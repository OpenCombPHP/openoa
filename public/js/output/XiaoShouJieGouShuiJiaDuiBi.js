jquery(function($){
	...
	function onLoad(){
		...
		$('#XiaoShouJieGouShuiJiaDuiBi_table')
		// 初始化 表格插件
		.datagrid({
			data: aXiaoShouJieGouShuiJiaDuiBiSheet
			, table: {
					editable: false
		...
		var getGraphData = function( row,column){
			...
			var value = aXiaoShouJieGouShuiJiaDuiBiSheet[row][column];
		...
		var bar5 = new RGraph.Bar('XiaoShouJieGouShuiJiaDuiBi_bar', data);
		
//		var bar5 = new RGraph.Bar('XiaoShouJieGouShuiJiaDuiBi_bar', [
...
//				   ]);
		...
		bar5.Set('chart.labels', ['居民', '工业', '商业', '基建']);
		...
		$('#BaoGao').bind('HuiZhi',function(e,data){
			if(data=='XiaoShouJieGouShuiJiaDuiBi_bar'){
				RGraph.Effects.Bar.Wave(bar5);
			}
		});
		//动画绘制
		...
		mapPies[year] = {
					col: 'A'
					, canvas: 'XiaoShouJieGouShuiJiaDuiBi_pie1'
					, data: []
					, tips: []
				} ;
		mapPies[year-1] = {
					col: 'B'
					, canvas: 'XiaoShouJieGouShuiJiaDuiBi_pie2'
					, data: []
					, tips: []
				} ;
