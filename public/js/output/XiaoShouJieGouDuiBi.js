jquery(function($){
	...
	function onLoad(){
		...
		$('#XiaoShouJieGouDuiBi_table')
		// 初始化 表格插件
		.datagrid({
			data: aXiaoShouJieGouDuiBiSheet
			, table: {
					editable: false
		...
		var getGraphData = function( row,column){
			...
			var value = aXiaoShouJieGouDuiBiSheet[row][column];
		...
		var bar5 = new RGraph.Bar('XiaoShouJieGouDuiBi_bar', data);
		buildGraph(bar5);
		...
		mapPies[year] = {
					col: 'A'
					, canvas: 'XiaoShouJieGouDuiBi_pie1'
					, data: []
					, tips: []
				} ;
		mapPies[year-1] = {
					col: 'B'
					, canvas: 'XiaoShouJieGouDuiBi_pie2'
					, data: []
					, tips: []
				} ;
		...
		for(var year in mapPies)
		{
			...
			for(var row=1;row<=5;row++)
			{
				var v = parseFloat(aXiaoShouJieGouDuiBiSheet[row][aInfo.col]) ;
				v = isNaN(v)?0:v ;
				aInfo.data.push(v) ;
