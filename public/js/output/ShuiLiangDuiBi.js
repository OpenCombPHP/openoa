jquery(function($){
	...
	function onLoad(){
		...
		$('#ShuiLiangDuiBi_table')
		// 初始化 表格插件
		.datagrid({
			data: aShuiLiangDuiBiSheet
			, table: {
					editable: false
		...
		var getGraphData = function( row,column){
			...
			var value = aShuiLiangDuiBiSheet[row][column];
...
//		var tips = [] ;
	...
	//	
		var bar5 = new RGraph.Bar('ShuiLiangDuiBi_bar', data);
		
//		var bar5 = new RGraph.Bar('ShuiLiangDuiBi_bar', [
...
//				   ]);
		...
		bar5.Set('chart.tooltips', toTips(data,' 万m³'));
		bar5.Set('chart.strokestyle', 'transparent');
		bar5.Set('chart.ymax', 3000);
		...
		mapPies[year] = {
					col: 'A'
					, canvas: 'ShuiLiangDuiBi_pie1'
					, data: []
					, tips: []
				} ;
		mapPies[year-1] = {
					col: 'B'
					, canvas: 'ShuiLiangDuiBi_pie2'
					, data: []
					, tips: []
				} ;
